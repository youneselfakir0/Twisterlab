#!/usr/bin/env python3
"""
TwisterLab API v5.2.3 — Smart Dispatch & Resilience
Implementing fuzzy parameter mapping in the AgentAdapter.
"""
...
"""
    path = BASE / "Dockerfile.v5.2.3"
    path.write_text(dockerfile, encoding="utf-8")

def step2_build():
    run(["docker", "build", "-f", "Dockerfile.v5.2.3", "-t", IMAGE, "."])

def step3_push():
    tar_path = BASE / "twisterlab-api-v5.2.3.tar"
    run(["docker", "save", IMAGE, "-o", str(tar_path)])
    ssh_host = "twisterlab-ubuntu"
    remote_tar = "/tmp/twisterlab-api-v5.2.3.tar"
...
def main():
    ...
    print(f"\n✅ TwisterLab API {NEW_VERSION} (Smart Dispatch) deployed.")
