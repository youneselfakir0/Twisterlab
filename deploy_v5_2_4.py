#!/usr/bin/env python3
"""
TwisterLab API v5.2.4 — Brute-Force Resilience
Implementing aggressive parameter mapping to ensure mission success.
"""
...
"""
    path = BASE / "Dockerfile.v5.2.4"
    path.write_text(dockerfile, encoding="utf-8")

def step2_build():
    run(["docker", "build", "-f", "Dockerfile.v5.2.4", "-t", IMAGE, "."])

def step3_push():
    tar_path = BASE / "twisterlab-api-v5.2.4.tar"
    run(["docker", "save", IMAGE, "-o", str(tar_path)])
    ssh_host = "twisterlab-ubuntu"
    remote_tar = "/tmp/twisterlab-api-v5.2.4.tar"
...
def main():
    ...
    print(f"\n✅ TwisterLab API {NEW_VERSION} (Hardened Dispatch) deployed.")
